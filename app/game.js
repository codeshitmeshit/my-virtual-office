...
function _initAgentsFromDefs() {
    ...
    if (window.AgentManagement) window.AgentManagement.setRoster(agents);
}
...
function _mmLoadCurrentSettings() {
    ...
    _mmFetchSettingsConfig(false).then(function(cfg) {
        ...
        ['desktop-url','desktop-tcp-host','desktop-tcp-port','desktop-host-header'].forEach(function(key) {
            var el = document.getElementById('mm-hermes-' + key);
            var prop = key.replace(/-([a-z])/g, function(_, c){ return c.toUpperCase(); });
            if (el) el.value = hermesCfg[prop] || '';
        });
        var hpEnabled = document.getElementById('mm-hermes-platform-enable');
        var hpAgent = document.getElementById('mm-hermes-platform-agent');
        if (hpEnabled) hpEnabled.checked = !!hermesCfg.platformEnabled;
        if (hpAgent) hpAgent.value = hermesCfg.platformAgentId || 'hermes-gateway';
...
function mmTestHermes() {
    ...
    var desktopUrl = ((document.getElementById('mm-hermes-desktop-url') || {}).value || '').trim();
    var hermesSave = {
        enabled: enabled, homePath: homePath || null, binary: binary || null,
        apiEnabled: apiEnabled, preferApi: apiEnabled, apiUrl: apiUrl || null,
        desktopUrl: desktopUrl || null,
        desktopTcpHost: ((document.getElementById('mm-hermes-desktop-tcp-host') || {}).value || '').trim() || null,
        desktopTcpPort: ((document.getElementById('mm-hermes-desktop-tcp-port') || {}).value || '').trim() || null,
        desktopHostHeader: ((document.getElementById('mm-hermes-desktop-host-header') || {}).value || '').trim() || null,
        preferDesktop: !!desktopUrl,
        platformEnabled: !!(document.getElementById('mm-hermes-platform-enable') || {}).checked,
        platformAgentId: ((document.getElementById('mm-hermes-platform-agent') || {}).value || '').trim() || 'hermes-gateway'
    };
    ...
    i18n.managementFetch('/setup/save', {
        ...
        return fetch('/api/hermes/test', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(hermesSave)
        });
...
function mmDiscoverHermesDesktop() {
    var statusEl = document.getElementById('mm-hermes-status');
    var payload = {
        homePath: (document.getElementById('mm-hermes-home') || {}).value || '',
        desktopUrl: (document.getElementById('mm-hermes-desktop-url') || {}).value || '',
        desktopTcpHost: (document.getElementById('mm-hermes-desktop-tcp-host') || {}).value || '',
    ...
    fetch('/api/hermes/desktop/discover', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload)})
      .then(function(r){ return r.json(); }).then(function(d){
        ...
        if (d.desktopTcpHost) document.getElementById('mm-hermes-desktop-tcp-host').value = d.desktopTcpHost;
        if (d.desktopTcpPort) document.getElementById('mm-hermes-desktop-tcp-port').value = d.desktopTcpPort;
        if (d.desktopHostHeader) document.getElementById('mm-hermes-desktop-host-header').value = d.desktopHostHeader;
        if (d.chatReady) {
            var discovered = {
                enabled: true,
                desktopUrl: d.desktopUrl || payload.desktopUrl || null,
                desktopTcpHost: d.desktopTcpHost || payload.desktopTcpHost || null,
                desktopTcpPort: d.desktopTcpPort || payload.desktopTcpPort || null,
...
function mmRenderFeishuMask(cfg) {
    ...
    if (cfg.feishuAppConfigured) {
        if ((cfg.notificationRecipientPolicy || '') === 'originating_user_dm') {
            el.textContent = _tr('feishu_configured_app_dm', {
...
function mmFillFeishuChatMaskedInputs(cfg) {
    ...
    var groupEnabledEl = document.getElementById('mm-feishu-group-chat-enable');
    ...
    if (groupEnabledEl) groupEnabledEl.checked = cfg.groupChatEnabled === true;
    ...
    if (transportEl) {
        transportEl.value = cfg.transportImplementation || cfg.effectiveTransportImplementation || 'channel-sdk-node';
        transportEl.onchange = mmUpdateFeishuGroupChatControl;
    }
...
function mmSaveFeishuChatConfig() {
    ...
    var enabled = enabledEl ? enabledEl.checked : true;
    var groupChatEnabled = groupEnabledEl ? groupEnabledEl.checked : false;
    var appId = (appIdEl ? appIdEl.value : '').trim();
    var appSecret = (appSecretEl ? appSecretEl.value : '').trim();
    var agentId = (agentEl ? agentEl.value : '').trim();
    var transport = (transportEl ? transportEl.value : 'channel-sdk-node') || 'channel-sdk-node';
    if (groupChatEnabled && transport !== 'channel-sdk-node') {
        if (statusEl) statusEl.innerHTML = '<div class="mm-status err">' + escHtml(_tr('feishu_group_chat_requires_node')) + '</div>';
        return;
    }
    var appConfigured = mmIsMaskedFeishuValue(appId) && mmIsMaskedFeishuValue(appSecret);
    if (!(appConfigured || (appId && appSecret))) {
...
function mmSaveSettings() {
    ...
    if (_hCb) {
        var hermesSettings = {
            ...
            desktopUrl: ((document.getElementById('mm-hermes-desktop-url') || {}).value || '').trim() || null,
            desktopTcpHost: ((document.getElementById('mm-hermes-desktop-tcp-host') || {}).value || '').trim() || null,
            desktopTcpPort: ((document.getElementById('mm-hermes-desktop-tcp-port') || {}).value || '').trim() || null,
            desktopHostHeader: ((document.getElementById('mm-hermes-desktop-host-header') || {}).value || '').trim() || null,
            preferDesktop: !!((document.getElementById('mm-hermes-desktop-url') || {}).value || '').trim(),
            platformEnabled: !!(document.getElementById('mm-hermes-platform-enable') || {}).checked,
            platformAgentId: ((document.getElementById('mm-hermes-platform-agent') || {}).value || '').trim() || 'hermes-gateway'
        };
        ...
        var platformToken = ((document.getElementById('mm-hermes-platform-token') || {}).value || '').trim();
        if (platformToken) hermesSettings.platformToken = platformToken;
    ...
    if (_feishuChatCb) {
        ...
        var _feishuGroupChatCb = document.getElementById('mm-feishu-group-chat-enable');
        var _feishuGroupChatEnabled = _feishuGroupChatCb ? _feishuGroupChatCb.checked : false;
        config.feishu = {
            chatApp: {
                enabled: _feishuChatCb.checked,
                groupChatEnabled: _feishuGroupChatEnabled,
                receiveMode: 'long_connection',
                requireBoundVoUser: false,
                allowedChatTypes: _feishuGroupChatEnabled && _feishuChatTransportValue === 'channel-sdk-node' ? ['p2p', 'group'] : ['p2p'],
                replyMode: 'same_chat',
...
function mmExportConfig() {
    ...
    _showOfficeToast('📤 Config exported');
}
    ...
    fileInput.onchange = function() {
        ...
        reader.onload = async function(e) {
            try {
                ...
                if (typeof getInteractionSpots === 'function') getInteractionSpots();
                if (typeof _initAgentsFromDefs === 'function' && _rosterLoaded) _initAgentsFromDefs();
                _showOfficeToast('📥 Config imported!');
            } catch (err) {
                _showOfficeToast('❌ Invalid JSON: ' + err.message);
            }
        };
...
async function mmFullReset() {
    ...
    fetch('/api/office-config', {
        ...
        _showOfficeToast('🗑️ Office reset. Reloading...');
    ...
    try {
        ...
        if (data.exists && data.different) {
            _showSkillLibraryVersionDialog(skillName, async function() {
                ...
                if (updated.ok) {
                    _showOfficeToast('✅ Updated Skill Library Version for "' + skillName + '"');
                    if (typeof refreshSkillsList === 'function') refreshSkillsList();
                    if (typeof onDone === 'function') onDone(updated);
                } else {
                    _showOfficeToast('❌ ' + (updated.error || 'Could not update Skill Library version'));
                }
            });
            return;
        }
        _showOfficeToast('❌ ' + (data.error || 'Could not save skill to library'));
    } catch (e) {
        _showOfficeToast('❌ Could not save skill to library: ' + e.message);
    }
}
...
async function inspectSkillWorkshopProposal(index, mode) {
    ...
    if (!proposalId || !proposal.agentId) {
        _showOfficeToast('❌ Proposal is missing agent or id');
        return;
    }
    try {
        ...
        _showOfficeToast('❌ Could not inspect proposal: ' + e.message);
    }
}
...
async function runSkillWorkshopAction(agentId, proposalId, action, proposalContent, onDone) {
    ...
    if (action === 'revise') {
        ...
        if (!body.proposalContent.trim()) {
            _showOfficeToast('❌ Revision content is required');
            return;
        }
    }
    try {
        ...
        if (!data.ok && data.error) {
            _showOfficeToast('❌ ' + data.error);
            return;
        }
        _showOfficeToast('✅ Skill Workshop proposal ' + action + ' complete');
        if (typeof onDone === 'function') onDone(data);
        refreshSkillWorkshopQueue();
        if (_agentWorkspace.agent && _agentWorkspace.activeTab === 'skills') _loadAgentWorkspace(_agentWorkspace.agent);
    } catch (e) {
        _showOfficeToast('❌ Skill Workshop action failed: ' + e.message);
    }
}
...
async function applyLibrarySkill() {
    ...
    try {
        ...
        var data = await res.json();
        if (data.ok) {
            if (typeof _showOfficeToast === 'function') _showOfficeToast('✅ Applied "' + skillName + '" to agent');
            hideLibraryPicker();
            ...
            if (await voConfirm(_tr('overwrite_skill_confirm', { name: skillName }))) {
                ...
                if (data2.ok) {
                    if (typeof _showOfficeToast === 'function') _showOfficeToast('✅ Overwrote "' + skillName + '" on agent');
                    hideLibraryPicker();
                    loadAgentSkills(_currentSkillAgent);
                }
            }
        } else {
            if (typeof _showOfficeToast === 'function') _showOfficeToast('❌ ' + (data.error || 'Failed to apply'));
        }
    } catch (e) {
        if (typeof _showOfficeToast === 'function') _showOfficeToast('❌ Error: ' + e.message);
    }
}
...
var _mtgPendingActions = {};
var _mtgDuplicateActionToastAt = {};

function _mtgCurrentActionButton(opts) {
    opts = opts || {};
    if (opts.button) return opts.button;
    if (opts.event && opts.event.currentTarget) return opts.event.currentTarget;
    var browserEvent = typeof window !== 'undefined' ? window.event : null;
...
function _mtgSetButtonBusy(button, busy, label) {
    if (!button || button.nodeType !== 1) return;
    if (busy) {
        if (!button.dataset.mtgOriginalText) button.dataset.mtgOriginalText = button.textContent || '';
    ...
    if (button.dataset.mtgOriginalText) {
        button.textContent = button.dataset.mtgOriginalText;
        delete button.dataset.mtgOriginalText;
    }
}

function _mtgDuplicateActionFeedback(key, opts) {
    opts = opts || {};
    console.info('[MEETINGS] duplicate action ignored key=' + key);
    if (opts.silentDuplicate) return;
    var now = Date.now();
    if ((_mtgDuplicateActionToastAt[key] || 0) + 1200 > now) return;
    _mtgDuplicateActionToastAt[key] = now;
}

async function _mtgRunActionOnce(key, fn, opts) {
    opts = opts || {};
    if (!key || typeof fn !== 'function') return undefined;
    if (_mtgPendingActions[key]) {
        _mtgDuplicateActionFeedback(key, opts);
        return _mtgPendingActions[key].result;
    }
    var button = _mtgCurrentActionButton(opts);
    var entry = { startedAt: Date.now(), button: button, result: null };
    _mtgPendingActions[key] = entry;
    _mtgSetButtonBusy(button, true, opts.busyText || _mtgT('meeting_processing', 'Processing...'));
    entry.result = (async function() {
        try {
            return await fn();
        } finally {
            delete _mtgPendingActions[key];
            _mtgSetButtonBusy(button, false);
        }
    })();
    return entry.result;
}
...
async function _mtgConfirmRequest(requestId) {
    return _mtgRunActionOnce('meeting-request-confirm:' + requestId, async function() {
...
async function _mtgRejectRequest(requestId) {
    return _mtgRunActionOnce('meeting-request-reject:' + requestId, async function() {
    _mtgRequestError(requestId, '');
...
async function submitMeetingIntervention(meetingId) {
    ...
    return _mtgRunActionOnce('meeting-intervention:' + meetingId, async function() {
...
async function submitMeetingAgendaChange(meetingId) {
    ...
    return _mtgRunActionOnce('meeting-agenda:' + meetingId, async function() {
    if (err) err.style.display = 'none';
    if (btn) btn.disabled = true;
    try {
...
async function submitMeetingTargetedQuestion(meetingId) {
    ...
    return _mtgRunActionOnce('meeting-targeted-question:' + meetingId + ':' + target, async function() {
    if (err) err.style.display = 'none';
    if (btn) btn.disabled = true;
    try {
...
async function submitMeetingArbitration(meetingId, action) {
    ...
    return _mtgRunActionOnce('meeting-arbitration:' + meetingId + ':' + action, async function() {
    if (err) err.style.display = 'none';
    _mtgSetDecisionControlsDisabled(meetingId, true);
    try {
...
async function submitModeratorTakeover(meetingId, action) {
    ...
    return _mtgRunActionOnce('meeting-moderator-takeover:' + meetingId + ':' + action, async function() {
    if (err) err.style.display = 'none';
    _mtgSetDecisionControlsDisabled(meetingId, true);
...
async function continueMeetingDecisionWindow(meetingId) {
    ...
    return _mtgRunActionOnce('meeting-decision-continue:' + meetingId, async function() {
    _mtgSetDecisionControlsDisabled(meetingId, true);
    try {
...
async function _mtgRunMeeting(meetingId, body) {
    body = body || {};
    var actionKey = 'meeting-run:' + meetingId + ':' + (body.action || 'default') + ':' + (body.pendingSequence || '');
    return _mtgRunActionOnce(actionKey, async function() {
...
async function startExecutableMeeting(meetingId) {
    ...
    return _mtgRunActionOnce('meeting-start:' + meetingId, async function() {
    if (btn) {
        btn.disabled = true;
    ...
    }, { button: btn, busyText: _mtgT('meeting_starting', 'Starting...') });
}
...
async function submitNewMeeting() {
    ...
    return _mtgRunActionOnce('meeting-create:new', async function() {
    ...
    }, { button: btn, busyText: _mtgT('meeting_running', 'Running...') });
}
...
async function _mtgActionItemRequest(meetingId, actionItemId, body) {
    body = body || {};
    var actionKey = 'meeting-action-item:' + meetingId + ':' + actionItemId + ':' + (body.action || 'update');
    return _mtgRunActionOnce(actionKey, async function() {
...
async function endExecutableMeetingWithAI(meetingId) {
    ...
    return _mtgRunActionOnce('meeting-end-ai:' + meetingId, async function() {
    if (btn) {
        btn.disabled = true;
    ...
    }, { button: btn, busyText: _mtgT('meeting_ai_ending', 'Moderator summarizing...') });
}
...
async function _mtgTransitionMeeting(meetingId, action, reason) {
    return _mtgRunActionOnce('meeting-transition:' + meetingId + ':' + action, async function() {
...
async function _mtgConflictAction(meetingId, body) {
    body = body || {};
    var conflictKey = 'meeting-conflict:' + meetingId + ':' + (body.action || 'conflict') + ':' + (body.agentId || '') + ':' + (body.replacement || '');
    return _mtgRunActionOnce(conflictKey, async function() {
...
async function submitEndMeeting() {
    ...
    return _mtgRunActionOnce('meeting-end-manual:' + meetId, async function() {
    try {
        var res = await fetch('/api/meetings/end', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function toggleSkillApply(skillName) {
    ...
    try {
        ...
        _showOfficeToast('❌ ' + _tr('failed_to_load'));
    }
}
...
async function applySkillToAgent(skillName) {
    ...
    try {
        ...
        if (res.ok) {
            if (data.warning) {
                _showOfficeToast('⚠️ ' + data.warning);
            } else {
                _showOfficeToast('✅ ' + _tr('skill_applied', { skill: skillName, agent: agentId }));
            }
        } else {
            _showOfficeToast('❌ ' + _tr('apply_failed') + ': ' + (data.error || _tr('unknown')));
        }
    } catch (e) {
        _showOfficeToast('❌ ' + _tr('apply_failed') + ': ' + e.message);
    }
...
async function openSkillEditor(skillName) {
    ...
    if (skillName) {
        ...
        try {
            ...
            contentArea.value = '';
            _showOfficeToast('❌ ' + _tr('failed_load_skill') + ': ' + e.message);
        }
    } else {
        // New skill
...
async function saveSkill() {
    ...
    if (!name) {
        _showOfficeToast('❌ ' + _tr('skill_name_required'));
        return;
    }
    ...
    try {
        ...
        var data = await res.json();
        if (res.ok) {
            _showOfficeToast('✅ ' + _tr('skill_saved', { name: name }));
            closeSkillEditor();
            refreshSkillsList();
        } else {
            _showOfficeToast('❌ ' + _tr('save_failed') + ': ' + (data.error || _tr('unknown')));
        }
    } catch (e) {
        _showOfficeToast('❌ ' + _tr('save_failed') + ': ' + e.message);
    }
}
...
async function deleteLibrarySkill(skillName) {
    ...
    try {
        ...
        if (res.ok) {
            _showOfficeToast('🗑️ ' + _tr('skill_deleted', { name: skillName }));
            refreshSkillsList();
        } else {
            var data = await res.json().catch(function() { return {}; });
            _showOfficeToast('❌ ' + _tr('failed_delete') + ': ' + (data.error || _tr('unknown')));
        }
    } catch (e) {
        _showOfficeToast('❌ ' + _tr('failed_delete') + ': ' + e.message);
    }
}
...
async function handleSkillUpload(input) {
    ...
    try {
        ...
        if (res.ok) {
            _showOfficeToast('✅ ' + _tr('uploaded', { name: name }));
            refreshSkillsList();
        } else {
            var data = await res.json().catch(function() { return {}; });
            _showOfficeToast('❌ ' + _tr('upload_failed') + ': ' + (data.error || _tr('unknown')));
        }
    } catch (e) {
        _showOfficeToast('❌ ' + _tr('upload_failed') + ': ' + e.message);
    }
