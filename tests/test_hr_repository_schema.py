...
def test_construction_has_no_side_effect_and_initialize_creates_fixed_authority(tmp_path):
    ...
    with sqlite3.connect(repo.path) as connection:
        ...
        report_columns = {
            row[1] for row in connection.execute("PRAGMA table_info(daily_reports)")
        }
    assert "skill_readiness" not in agent_columns
    assert "grant_readiness" not in agent_columns
    assert "normalized_json" not in report_columns
    assert "normalizer_id" not in report_columns
    assert "normalized_at" not in report_columns
...
def test_schema_metadata_matches_sqlite_user_version(tmp_path):
    ...
    assert metadata == {
        ...
        "last_migration": "remove_daily_report_legacy_normalization_columns",
...
def test_existing_v2_repository_drops_legacy_authorization_table_columns_and_rows(tmp_path):
    ...
    assert upgraded.initialize().schema_version == 8
...
def test_failed_upgrade_rolls_back_all_schema_and_metadata_changes(tmp_path):
    ...
    upgraded = repository(
        tmp_path,
        migrations=(*DEFAULT_MIGRATIONS, HRMigration(9, "failing_upgrade", fail_after_ddl)),
    )
...
def test_concurrent_initializers_serialize_without_duplicate_schema(tmp_path):
    ...
    assert results[0].schema_version == results[1].schema_version == 8
    assert set(repo.table_names()) == EXPECTED_TABLES
