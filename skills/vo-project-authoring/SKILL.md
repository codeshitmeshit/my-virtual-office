...
把用户意图整理成可读的自然语言项目方案。只有用户明确确认当前版本后，才把它转换成结构化数据并直接创建真实项目。不要把方案提交成后台草稿；Agent 创建默认启用 Project Execution，但创建后仍保持未启动，只有用户显式要求执行时才开始。
...
结构化 `project` 必须包含完整 columns/tasks、角色、显式 reviewer recommendation、维护模式、执行配置以及一致的模板/周期配置：

- 每个任务的 `description` 必须携带确认方案中的输入、输出、执行说明和风险/讨论，使用稳定段落：`输入：...`、`输出：...`、`执行说明：...`、`风险/讨论：...`。没有风险或讨论时写“无”；未知输入或输出写“待确认”。
- 每个任务表格中的“验收标准”必须逐项转换为该任务的结构化 `checklist`，格式为 `[{"text":"...","done":false}]`；多条标准拆成多个 item，不能只留在确认文本。
- checklist 只放交付物验收标准。输入、输出模板、执行步骤、会议行动项、风险、讨论点和上下文分别写入 `description` 或对应 meeting 字段，不得自动提升为 checklist。
...
- 默认不要创建可复用模板。只有用户明确说“创建模板 / 保存成模板 / 复用模板 / template.mode=create / 引用某个 templateId”等等价语义，且确认方案里明确写出模板动作时，才可使用 `"template":{"mode":"create"}` 或 `"template":{"mode":"reference"}`。
- 周期也是项目属性；需要自动生成独立项目实例时才创建或引用模板。只想让已有项目按固定时间运行时，不要使用模板 recurrence，改用下文的项目 scheduled-cron。
- schedule 支持 `cron`（5–7 段）或 `every`（`everyMs >= 60000`），并使用有效 IANA timezone。周期项目必须携带 `recurrence.executionMode`；`create_only` 只创建未启动实例，`create_and_execute` 表示用户已另行确认每个实例创建后自动执行。
  ...
  -d '{
    ...
    "project":{
      ...
      "tasks":[{
        ...
        "checklist":[{"text":"发布材料完整且可验证","done":false}]
      }],
      "template":{"mode":"none"},
...
```json
{
  ...
  "task": {
    "title": "生成日报",
    "description": "输入：VO 中 AI 的当日工作记录和项目上下文\n\n输出：日报正文和待用户确认的摘要\n\n执行说明：汇总变化、风险和后续动作，保持来源可追溯。\n\n风险/讨论：无",
    "columnId": "backlog",
    "priority": "medium",
    "responsibleActor": {"type": "agent", "id": "codex-local"},
    "executorActor": {"type": "agent", "id": "codex-local"},
    "reviewerRecommendation": {"recommended": false, "triggers": []},
    "checklist": [{"text": "日报正文覆盖当日关键工作、风险和后续动作", "done": false}]
  }
}
```
