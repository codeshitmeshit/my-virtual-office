...
- 目标是 `codex-local` 或 `providerKind=codex` 的 Codex 协作者：使用 本地 `/skills/vo-codex-communication/SKILL.md`。
...
- 需要读取或推进项目/任务执行、Project Execution、review、验收、阻塞或项目 artifact：使用 本地 `/skills/vo-project-workflow/SKILL.md`。
...
- 可以通过 本地 `/skills/vo-agent-communication/SKILL.md` 或 本地 `/skills/vo-codex-communication/SKILL.md` 完成的普通协作。
- 只是为了通知另一个 AI 或转交信息。
...
- 普通通信已先识别目标 `providerKind`，并在 本地 `/skills/vo-agent-communication/SKILL.md` 和 本地 `/skills/vo-codex-communication/SKILL.md` 之间选择其一。
- 普通协作已优先考虑专用通信 skill，会议只用于正式多方决策或需要用户确认上下文的场景。
- 项目执行、Agent workspace、已确认会议执行已分别路由到 本地 `/skills/vo-project-workflow/SKILL.md`、本地 `/skills/vo-agent-workspace/SKILL.md`、本地 `/skills/vo-meeting-execution/SKILL.md`。
