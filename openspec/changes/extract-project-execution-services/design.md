...
- 异常与兼容：锁顺序固定为 registry guard → project lock → store commit lock；不得持 binding lock 获取 project/store lock，不得在 mutator 内嵌套 `repo.update`。兼容期内所有 `_save_projects` / `PROJECT_STORE.save_all` writer 必须接入同一个 commit coordinator，禁止携带 stale 全量 snapshot 直接保存。所有目标 project 写入（包括 activity、comment、reviewCheck 等非 execution 字段）必须先取得同一个 project lock；能声明 owned fields 的，在 commit lock 内重读 latest 后字段合并；无法证明安全字段合并的 legacy 命令，最小改为单次 `repo.update`，但其业务逻辑仍可保留在 `_wf_*`。因此同项目不存在绕过 project lock 的 writer，跨项目则由 store commit lock 防止全量覆盖。store 异常继续向上层兼容映射。
...
- [ ] `rg` 清点全部 `_save_projects`/`PROJECT_STORE.save_all` writer；不存在绕过 commit coordinator 的 stale 全量保存。
