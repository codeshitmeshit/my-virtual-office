...
带条件通过。方案能够修复 VO skill 来源分裂、未安装到 agent workspace、OpenClaw 检测误报和通信不可追踪问题。进入实现的条件是 tasks 必须包含 managed-copy 冲突保护、原子/幂等同步、创建失败语义、检测兼容性测试，以及至少一次真实 OpenClaw 委派验收。
...
- 安全：workspace 路径来自 agent 配置。同步前必须做 realpath 边界检查，仅允许写入已确认 OpenClaw workspace 下的保留 skill 路径。
...
- 可回滚性：旧 skill 名可能仍被部分 agent 记忆。迁移顺序必须先安装 canonical，再删除已知 managed legacy copy；冲突内容不得直接删除。
