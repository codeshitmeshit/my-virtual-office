...
- Require every completed task to leave a fixed Markdown final-result artifact and expose prior-stage result indexes to later-stage execution prompts so staged work can discover upstream conclusions without guessing where to look.
- Persist an internal new-project orchestration marker in the canonical Markdown project frontmatter without exposing a user-visible badge.
...
- Project Markdown-store serialization/materialization and every project-creation path that must set the internal orchestration marker.
