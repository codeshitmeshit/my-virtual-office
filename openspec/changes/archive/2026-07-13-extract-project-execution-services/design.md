...
- [x] 每个新 Service 不 import `server.py`/`OfficeHandler`。
- [x] `rg` 清点全部 `_save_projects`/`PROJECT_STORE.save_all` writer；迁移期 `_save_projects` 仅为 Repository 三方合并 delegate，不存在绕过 coordinator 的 stale 全量保存。
- [x] Writer inventory 覆盖 CRUD、execution、review/acceptance、Feishu、Meeting、Cron、archive/recovery、legacy `_wf_*` 与 delete_project，并标注迁移策略。
- [x] 每片有 characterization、Service、adapter 与性能基线测试。
- [x] load/save/provider 次数不高于基线，目标冗余路径下降。
- [x] 慢工作不持 project lock，陈旧结果不可覆盖。
- [x] API、存储、事件、通知和 Provider 契约通过回归。
- [x] 发现 bug 时先复现、补规格/测试并记录行为差异。
...
三路独立评审确认采用按项目原子更新、显式 Service ports、稳定本地通知 intent + best-effort 外部投递，以及 Cron 双写失败诊断/补偿方案。评审发现的并发、Artifact 根目录竞态、Cron 补偿与证据一致性问题均已补回归并修复。

#### 结论

技术方案与实现范围已确认；代码、性能、兼容性和本地启动/回滚门禁通过。真实 staging 演练仍是正式上线与归档前置条件，不影响代码推送。
...
复审确认一审阻断问题已关闭；最终 push gate 以未推送 commits 的四路独立 CR 为准。
...
- 通知一致性使用稳定本地 side-effect intent 与现有 best-effort 外部投递；本期不引入 durable outbox 或常驻投递 worker。
- Cron Gateway/binding 双写失败执行同步补偿并保留 diagnostic reconciliation state；本期不引入常驻 reconcile worker。
