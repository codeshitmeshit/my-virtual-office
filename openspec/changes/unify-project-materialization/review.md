...
- 本地 `office.py --proj create`/`add-task` 可能被脚本调用：保留 CLI 参数、built-in template 选择、输出语义和存储位置，只替换对象拼装权威。
- confirmed Agent maintenance 的 `create_task` 受 grant、用户确认和 root transaction 保护：迁移时只替换 Task materialization，不改变权限、冲突或原子提交语义。
...
- 覆盖本地 `office.py --proj create` 的空项目/built-in template，以及 `add-task` 的列、顺序、参数解析、打印结果和持久化兼容。
- 覆盖 confirmed Agent maintenance `create_task` 的授权、actor validation、ID conflict、完整 Task 默认字段和原子回滚。
