...
- Capacity 1 remains supported. Capacity 2 is approved by the deterministic multiplexing fixture: two native threads interleave start, reasoning, approval, user input, cancellation, completion, result delivery, and cleanup without cross-delivery. The configured product default is now 8; deterministic admission coverage proves eight bounded slots and explicit ninth-turn rejection, but native protocol interleaving and real-Provider saturation above capacity 2 remain unproven.
