...
- [x] 5.3 [MP-5] 新建 `app/services/project_completion_report_worker.py`，使用 `services.periodic_timer.PeriodicTimer` 每 15 秒扫描、每批最多 claim 10 条，串联 artifact collector、generation、storage 与 delivery ports；测试立即扫描、due filter、批量上限、进程重启恢复、单条异常隔离和 stop 行为；提交：`feat(reporting): add persistent completion report worker`。
