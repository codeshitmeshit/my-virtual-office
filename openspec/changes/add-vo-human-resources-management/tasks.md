...
- [ ] 13.5 Deploy to the development machine with `VO_HR_ENABLED=0` and the persisted HR page schedule disabled before lifecycle enablement; run existing VO, archive-manager, Archive Room, project, meeting, and provider smoke tests and record a clean pre-enable baseline.
...
- [ ] 13.7 Enable directory, introduction, and `vo-agent-hr`; verify real Agent discovery, HR self-exclusion, Agent self-description, `/skills` catalog exposure without Agent workspace copies or credentials, trusted caller-ID queries across Providers, denial paths, and best-effort access audit.
...
- [ ] 13.9 Execute rollback rehearsal by disabling the automatic schedule from the HR page and then disabling HR, allowing claims to settle/expire, preserving HR data and Agent identity, and proving Archive Room and existing VO workflows remain operational; record restoration steps and results.
- [ ] 13.10 Run final OpenSpec validation, assemble specification-to-test traceability and all local/development-machine evidence, document uncovered items, and prepare the separate test-result confirmation package without archiving the change.
